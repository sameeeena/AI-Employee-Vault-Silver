#!/usr/bin/env python3
"""
Robust Filesystem Watcher for Silver Tier AI Employee
...
class SilverTierFileHandler(FileSystemEventHandler):
    """Handles file system events for the Silver Tier AI Employee."""
    ...
    def __init__(self, inbox_dir: str, needs_action_dir: str, log_manager: IngestionLogManager):
        ...
        self.processed_files: Set[str] = set()  # Track recently processed files
    ...
    def _process_new_file(self, file_path: Path):
        ...
        try:
            ...
            try:
                ...
                # Add to processed files set (with timestamp for cleanup)
                self.processed_files.add(str(file_path))

                # Clean up old entries from processed set periodically
                if len(self.processed_files) > 1000:
                    # Keep only recent entries (last 10 minutes)
                    recent_threshold = time.time() - 600  # 10 minutes
                    self.processed_files = {
                        f for f in self.processed_files
                        if os.path.exists(f) and os.path.getmtime(f) > recent_threshold
                    }
...
def main():
    """Main entry point for the Silver Tier Filesystem Watcher."""
    # Define absolute paths
    base_dir = Path(__file__).parent.absolute()
    inbox_dir = base_dir / "Inbox"
    needs_action_dir = base_dir / "Needs_Action"
    log_dir = base_dir / "logs"
    log_file_path = log_dir / "ingestion_log.md"
    ...
    event_handler = SilverTierFileHandler(
        str(inbox_dir),
    ...
    try:
        observer.start()
        log_manager.logger.info("Silver Tier Filesystem Watcher started")
        log_manager.logger.info(f"Monitoring: {inbox_dir}")
        log_manager.logger.info("Press Ctrl+C to stop...")

        # Print initial system status
        print("\n" + "="*50)
        print("SILVER TIER FILESYSTEM WATCHER STATUS")
        print("="*50)
        ...
        observer.stop()
        log_manager.logger.info("Silver Tier Filesystem Watcher stopped by user")
        ...
        observer.join()
        log_manager.logger.info("Silver Tier Filesystem Watcher terminated")
